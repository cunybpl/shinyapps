...
#### YOUR APP'S SERVER CODE GOES HERE ----------------------------------------
  ...
  output$tar_date_wiggy <- renderUI({
    tagList(
      textInput('tar_date', 'Traget Date', value ='', width = NULL, placeholder = 'yyyy-mm-dd')
    )
    })

  output$retro_start_wiggy <- renderUI({
    tagList(
      textInput('start_date', 'Retrofit Start Date', value ='', width = NULL, placeholder = 'yyyy-mm-dd')
    )
    })

  output$retro_end_wiggy <- renderUI({
    tagList(
      textInput('end_date', 'Retrofit End Date', value ='', width = NULL, placeholder = 'yyyy-mm-dd')
    )
    })
  ...
  baseline_batch <- reactive({
    task_id = get_task_id('/bema/modeler/baseline/basic/', query_params = baseline_query())
    fetch_request(paste('bema/results/', task_id, sep =''))$results
    })

  retro_batch <- reactive({
    response = get_batch_response('/bema/modeler/retrofit/basic/', query_params = retro_query())
    })
