...
#### YOUR APP'S SERVER CODE GOES HERE ----------------------------------------
  ...
  url_str <- reactive({construct_init_str(input$user_name, input$password)})

  observeEvent(input$login_button, {
      result = tryCatch({mongo('meta_data', 'json_data', url_str())},
  ...
  meta_data_df <- reactive({
    mongo('meta_data', 'json_db', url_str())
    })
