...
{ 
  ...
  { 
    ...
    for (model in model_list)
    { 
      if (inter_result[[model]]$stat_test$main_test == 'Pass' & inter_result[[model]]$stats[3,1] <= 0.25 & inter_result[[model]]$stats[2,1] >= 0.75)
      { 
  ...
  }else
  {
    ...
    for (model in model_list)
    {
      if (inter_result[[model]]$stat_test$main_test == 'Pass' & inter_result[[model]]$stats[3,1] <= 0.5 & inter_result[[model]]$stats[2,1] >= 0.75)
      { 
        best_result[[model]] = inter_result[[model]]
      }
    }
...
mahala_ellipse_func0 <- function(utility, energy){
...
mahala_ellipse_func <- function(utility, energy, bdbid_n){
  utility = subset(utility, utility$bdbid == bdbid_n & utility$energy_type == energy)
  ...
  CE <- outlier_func(utility, bdbid_n, energy) 
  outside <- data.frame(CE$final_df$outside)
  confidence_ellipse <- unlist(outside)

  new_utility <- cbind(utility, m_dist, confidence_ellipse)
  return(new_utility)
}
...
{ 
  if(is.null(df) | length(df) == 0 | !length(bdbid_n)){
    return(FALSE)
  }
